...
export default class Main {
    ...
    async deferredConstructor() {
        ...
        this.voronoiSpheres = new THREE.InstancedMesh( this.sphereGeo, new THREE.MeshStandardMaterial( { color: 0xff0000, roughness: 1, metalness: 0 } ), this.meshingParams );
    ...
    generateLattice(mesh){
        ...
        for(let i = 0; i < this.meshingParams.numPoints; i++){
            let x = Math.random() * boxSize.x + boxMinCorner.x; 
            let y = Math.random() * boxSize.y - boxMinCorner.y; 
            let z = Math.random() * boxSize.z + boxMinCorner.z; 

            //raycaster.set(tempVec.set(
            //    x - this.mesh.position.x,
            //    y - this.mesh.position.y,
            //    z - this.mesh.position.z), tempVec3);
            ...
            //}else{
            //    // Point is outside, reject this sample and try again
            //    i--;
            //}

            //this.voronoiSpheres.setMatrixAt( i, tempMat.compose( tempVec.set(x, y, z ), zeroRotation, scale));
            //this.latticeSpheres.setMatrixAt( i, tempMat.compose( tempVec.set(x, y, z ), zeroRotation, scale));
        }

        Voro3D.create(boxMinCorner.x, boxMaxCorner.x, boxMinCorner.y+0.0001, boxMaxCorner.y, boxMinCorner.z, boxMaxCorner.z, 1, 1, 1).then((voro) => {
            this.voro = voro;
